...
int inByte = 0,//serial buf
    num = 0;//buf counter
int x_openmv=0, y_openmv=0,z_openmv=0;
int x_uarm=0, y_uarm=0,z_uarm=0;
unsigned long times;
char buf[20],
     flag=0;
char color_sel=0;
...
//new algorithm
    ...
    y_uarm = x_openmv*(-0.7488)+12.391 + 107.5 + 15 -10;
    z_uarm=150-z_openmv*1.60;
...
unsigned char get_openmv_data()
{
  if (Serial1.available() > 0) 
  {
    ...
    if((inByte=='\n')&&(buf[0]=='x'))
    {
      ...
      counters++;//jump the letter y
      do{
        z_openmv = z_openmv*10;
        z_openmv += buf[counters++] - 48;
      }while(counters+1<num);
      Serial.write(x_openmv);
      Serial.write(y_openmv);
      Serial.write(z_openmv);
      num = 0;
      return 1;
    }
